...
router.get('/in-use-low-fuel', async (req, res) => {
  try {
    const cars = await carService.getInUseLowFuelCars();
    res.json(cars);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/reserved-unauthorized', async (req, res) => {
  try {
    const cars = await carService.getReservedUnauthorizedCars();
    res.json(cars);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.put('/service-update', async (req, res) => {
  try {
    const result = await carService.sendOldOrHighMileageCarsToService();
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.put('/relocate-popular', async (req, res) => {
  try {
    const result = await carService.relocatePopularCars();
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.delete('/:vin', async (req, res) => {
  try {
    const car = await carService.deleteCarByVin(req.params.vin);
